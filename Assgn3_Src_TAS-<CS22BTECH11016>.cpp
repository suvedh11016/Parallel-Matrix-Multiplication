...
int main()
{
    ...
    long long start_time_tas = getCurrentTimeMicroseconds();
    for (size_t i = 0; i < K; i++)
    {
        threads.emplace_back(tas_worker_function, rowInc);
    }
    for (auto &t : threads)
    {
        t.join();
    }
    long long end_time_tas = getCurrentTimeMicroseconds();

    // threads.clear();

    lock_guard<mutex> lock(mtx);
    ofstream output_file("out_tas.txt", ios::trunc);

    output_file << "Method: TAS" << endl;
    output_file << "Time taken: " << (end_time_tas - start_time_tas) << " microseconds" << endl;
