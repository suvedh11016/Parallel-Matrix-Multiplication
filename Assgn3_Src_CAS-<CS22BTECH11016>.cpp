...
int main()
{
    ...
    long long start_time_cas = getCurrentTimeMicroseconds();
    for (size_t i = 0; i < K; i++)
    {
        threads.emplace_back(cas_worker_function, rowInc);
    }
    for (auto &t : threads)
    {
        t.join();
    }
    long long end_time_cas = getCurrentTimeMicroseconds();



    lock_guard<mutex> lock(mtx);
    ofstream output_file("out_cas.txt", ios::trunc);


    output_file << "Method: CAS" << endl;
    output_file << "Time taken: " << (end_time_cas - start_time_cas) << " microseconds" << endl;
