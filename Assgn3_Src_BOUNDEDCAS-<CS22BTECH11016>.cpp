...
int main()
{
    ...
    long long start_time_bcas = getCurrentTimeMicroseconds();
    for (size_t i = 0; i < K; i++)
    {
        threads.emplace_back(bounded_cas_worker_function, i, rowInc);
    }
    for (auto &t : threads)
    {
        t.join();
    }
    long long end_time_bcas = getCurrentTimeMicroseconds();



    lock_guard<mutex> lock(mtx);
    ofstream output_file("out_bcas.txt", ios::trunc);





    output_file << "Method: BoundedCAS" << endl;
    output_file << "Time taken: " << (end_time_bcas - start_time_bcas) << " microseconds" << endl;
